...
def _load_runtime():
    ...
    try:
        ...
        from qsot.utils.loader import load_channels, load_rho0
    except ImportError as exc:
        print(f"Import Error: {exc}")
        print(f"sys.path: {sys.path}")
        sys.exit(1)

    return run, load_channels, load_rho0
...
def run_asdp(
    ...
    run, load_channels, load_rho0 = _load_runtime()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rho0",
        required=True,
        help="Path to initial state JSON/NPZ",
    )
    parser.add_argument(
        "--channels",
        required=True,
