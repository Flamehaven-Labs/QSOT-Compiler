...
def generate_fixture_data(name: str) -> Tuple[np.ndarray, List[dict]]:
    ...
    if name == "quantum_chaos":
        ...
        # [FIX] Use Superposition |+> to see dynamics
        # (Maximally mixed state 0.5*I is invariant)
        ...
        k_dep_json = [
            {"re": k.real.tolist(), "im": k.imag.tolist()}
            for k in [k_dep0, k_dep1, k_dep2, k_dep3]
        ]

        # Channel 2: Phase Damping (p=0.3)
        p_pd = 0.3
        k_pd0 = np.array(
            [[1, 0], [0, np.sqrt(1 - p_pd)]],
            dtype=np.complex128,
        )
        k_pd1 = np.array(
            [[0, 0], [0, np.sqrt(p_pd)]],
        ...
        k_pd_json = [
            {"re": k.real.tolist(), "im": k.imag.tolist()} for k in [k_pd0, k_pd1]
        ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="QSOT Data Loader & Generator",
    )
    parser.add_argument(
        "--export-rho0",
        help="Path to save rho0.json",
    )
    parser.add_argument(
        "--export-channels",
        help="Path to save channels.json",
    )
    parser.add_argument(
        "--fixture",
        help=(
            "Name of the fixture to generate "
            "(quantum_chaos | depolarizing_then_phase_damping)"
        ),
    )
